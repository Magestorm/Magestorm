...
{
    public class LauncherProgressBar : PictureBox
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            try
            {
                iSize = (Int32)Math.Floor(((double)Size.Width / Max) * Value);
            }
            catch (Exception){}

            ProgressBox.Size = Value < Max ? new Size(iSize - ImageBorderPixelSize, ProgressBox.Size.Height) : new Size(iSize - ((ImageBorderPixelSize * 2) + 1), ProgressBox.Size.Height);
            ProgressBorderBox.Size = new Size(iSize, ProgressBorderBox.Size.Height);

            if (ShowPercentage) e.Graphics.DrawString(Text, Font, new SolidBrush(Color.FromArgb(255, 90, 130, 240)), ((float)Size.Width / 2) - (Text.Length * 3), ((float)Size.Height / 2) - (ImageBorderPixelSize * 4));
        ...
        {
            if (ShowPercentage) e.Graphics.DrawString(Text, Font, new SolidBrush(Color.FromArgb(255, 95, 95, 95)), ((float)Size.Width / 2) - (Text.Length * 3), ((float)Size.Height / 2) - (ImageBorderPixelSize * 5));
        }
